...
func _state_attack(delta: float) -> void:
	...
	var to_player: Vector2 = (ship.global_position - global_position).normalized()
	var tangent: Vector2 = Vector2(-to_player.y, to_player.x) * strafe_dir

	# Back away if too close
	if dist < preferred_range_min:
		velocity = (-to_player + tangent * 0.5).normalized() * move_speed
	else:
		velocity = tangent * move_speed * 0.6

	# Fire at player
	if fire_cooldown <= 0.0:
		_fire_at(ship.global_position)
